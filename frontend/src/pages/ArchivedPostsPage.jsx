      ...
      <div className="max-w-2xl mx-auto space-y-6 mb-20">
          ...
          {posts.map(post => (
            <SocialCard key={post._id || post.id} id={post._id || post.id}
              author={{ name: post.author?.name || post.author?.username || 'Unknown', username: post.author?.username || 'unknown', avatar: post.author?.avatar || 'https://github.com/shadcn.png', timeAgo: post.updatedAt ? getTimeAgo(post.updatedAt) : 'Just now' }}
